...
/** Score the description dimension (0-40). */
function scoreDescription(desc: string): number {
  let score = 0;
  if (desc.length > 0) score += 10;
  if (desc.length > 100) score += 5;
  if (desc.length > 300) score += 5;
  if (desc.length > 500) score += 5;
  if (STRUCTURED_RE.test(desc)) score += 5;
  if (CHECKBOX_RE.test(desc) || AC_TEXT_RE.test(desc)) score += 5;
  if (CONTEXT_SECTION_RE.test(desc)) score += 5;
  return score;
}

/** Score the metadata dimension (0-30). */
function scoreMetadata(ticket: TicketData): number {
  let score = 0;
  if (ticket.storyPoints != null && ticket.storyPoints > 0) score += 10;
  if (ticket.labels.length > 0) score += 5;
  if (ticket.components.length > 0) score += 5;
  if (ticket.priority !== "Medium") score += 5;
  return Math.min(score, 30);
}

/** Score the process dimension (0-30). */
function scoreProcess(ticket: TicketData): number {
  let score = 0;
  const doneStatuses = new Set(["done", "closed", "resolved"]);
  if (doneStatuses.has(ticket.status.toLowerCase())) score += 10;
  if (ticket.changelog.length > 0) score += 5;
  ...
  if (statuses.size >= 3) score += 5;
  if (ticket.resolutionDate) score += 5;

  const descUpdated = ticket.changelog.some((c) => c.field === "description");
  if (descUpdated) score += 5;
...
export function scoreTicketQuality(ticket: TicketData): QualityScore {
  const description = scoreDescription(ticket.description ?? "");
  const metadata = scoreMetadata(ticket);
  const process = scoreProcess(ticket);
