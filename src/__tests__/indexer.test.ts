...
import { Spider, classifyPage } from "../lib/indexer.js";
...
describe("Spider", () => {
	...
	describe("crawl", () => {
		it("throws when neither spaceKey nor rootPageId provided", async () => {
			...
			await expect(spider.crawl({ maxDepth: 10, maxConcurrency: 1, includeLabels: [], excludeLabels: [] })).rejects.toThrow(
				"Either spaceKey or rootPageId must be provided",
			);
		});
	});
	...
	describe("crawlSpace", () => {
		it("indexes pages from a space", async () => {
			...
			const client = mockClient({
				...
				getPageFull: async (id: string) => pages.find((p) => p.id === id)!,
			});
