...
describe("registerSprintsTool", () => {
  ...
  describe("action=velocity", () => {
    it("returns velocity data", async () => {
      ...
      const sprints = getTool("sprints");
      const result = await sprints({ action: "velocity", sprintCount: 5 });
    ...
    it("includes bug rate when trendMetrics includes bugRate", async () => {
      ...
      const sprints = getTool("sprints");
      const result = await sprints({ action: "velocity", sprintCount: 5, trendMetrics: ["velocity", "bugRate"] });
      expect(result.isError).toBeUndefined();
      const text = result.content[0]?.text ?? "";
    ...
    it("includes scope change when trendMetrics includes scopeChange", async () => {
      ...
      const sprints = getTool("sprints");
      const result = await sprints({ action: "velocity", sprintCount: 5, trendMetrics: ["velocity", "scopeChange"] });
      expect(result.isError).toBeUndefined();
      const text = result.content[0]?.text ?? "";
  ...
  describe("action=retro", () => {
    it("returns comprehensive retrospective data", async () => {
      ...
      const sprints = getTool("sprints");
      const result = await sprints({ action: "retro" });
      const text = result.content[0]?.text ?? "";
    ...
    it("handles no closed sprints", async () => {
      ...
      const result = await sprints({ action: "retro" });
      expect(result.isError).toBe(true);
      expect(result.content[0]?.text).toContain("No closed sprints found");
    });
    ...
    it("shows scope creep section for issues added mid-sprint", async () => {
      ...
      const sprints = getTool("sprints");
      const result = await sprints({ action: "retro", sprintId: "15" });
      const text = result.content[0]?.text ?? "";
    ...
    it("shows time-in-status section for completed issues", async () => {
      ...
      const sprints = getTool("sprints");
      const result = await sprints({ action: "retro", sprintId: "16" });
      const text = result.content[0]?.text ?? "";
